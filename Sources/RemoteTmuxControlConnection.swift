...
final class RemoteTmuxControlConnection {
    ...
    private func handleStreamEnd(processGeneration generation: UInt64) async {
        ...
        case .reconnecting:
            ...
            // A reconnect that failed because the transport cannot run at all is not
            // transient either, and looping on it burns the backoff forever; it ends
            // the connection like a gone session rather than offering a login.
