...
extension RemoteTmuxController {
    ...
    private func awaitAuthenticationThenResume(host: RemoteTmuxHost) {
        ...
        authWaitTasks[key] = Task { @MainActor [weak self] in
            ...
            // A backstop behind the edge, not instead of it: even a live watch can silently miss
            // its event, and that is a total failure of the edge, so it gets a slow re-check
            // rather than trust.
            //
