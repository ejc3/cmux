...
rewrite_attach_url_for_simulator_host() {
  local attach_url="$1"
  local host_ip="$2"
  python3 - "$attach_url" "$host_ip" <<'PY'
import base64
import json
import sys
from urllib.parse import parse_qs, urlencode, urlparse, urlunparse

attach_url, host_ip = sys.argv[1:3]
...
[[ -n "$SIMULATOR_HOST_IP" ]] || { echo "Failed to determine simulator-reachable host IP" >&2; exit 1; }
ATTACH_URL="$(rewrite_attach_url_for_simulator_host "$ATTACH_URL" "$SIMULATOR_HOST_IP")"
...
python3 - "$IOS_XCTESTRUN" "$ATTACH_URL" "$SYNC_MARKER" \
...
path, attach_url, marker, email, password, dev_stack_token = sys.argv[1:7]
