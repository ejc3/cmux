...
final class cmuxUITests: XCTestCase {
    ...
    func testCloudSyncVideoTypesIntoRealMacTerminal() throws {
        ...
        let syncWorkspaceID = environment["CMUX_SYNC_WORKSPACE_ID"]
        ...
        if let syncWorkspaceID, !syncWorkspaceID.isEmpty {
            launchEnvironment["CMUX_SYNC_WORKSPACE_ID"] = syncWorkspaceID
            launchArguments.append("CMUX_SYNC_WORKSPACE_ID=\(syncWorkspaceID)")
        }
        ...
        try openSelectedWorkspaceIfNeeded(app, workspaceID: syncWorkspaceID)
    ...
    @MainActor
    private func openSelectedWorkspaceIfNeeded(_ app: XCUIApplication, workspaceID: String? = nil) throws {
        let terminalSurface = app.otherElements["MobileTerminalSurface"]
        if terminalSurface.waitForExistence(timeout: 8) {
            return
        }

        let rowQuery: XCUIElementQuery
        if let workspaceID, !workspaceID.isEmpty {
            rowQuery = app.descendants(matching: .any)
                .matching(NSPredicate(format: "identifier == %@", "MobileWorkspaceRow-\(workspaceID)"))
        } else {
            rowQuery = app.descendants(matching: .any)
    ...
    private enum Composer {
        ...
        /// Inline composer send button.
        static let sendButton = "MobileComposerSend"
