...
@Suite struct RemoteTmuxControlParserTests {
    ...
    @Test func mirrorTabReorderReordersNamedSubsetKeepingOtherTabsInPlace() {
        let a = UUID(), b = UUID(), c = UUID()
        // `mirrorTabReorder` reorders ONLY the tabs named in `requested` (in that
        // order); any tab in `current` that is NOT named — a coexisting local/browser
        // tab, or a mirror tab not yet reconciled — keeps its slot, so a local tab
        // doesn't defeat tmux-driven reordering of the mirror tabs around it.
        //
        // `a` is not named, so it stays in slot 0 while the named b,c reorder to c,b.
        #expect(RemoteTmuxSessionMirror.mirrorTabReorder(current: [a, b, c], requested: [c, b]) == [a, c, b])
        // A named subset already in its current relative order is a no-op → nil
        // (both a prefix subset and a suffix subset).
        #expect(RemoteTmuxSessionMirror.mirrorTabReorder(current: [a, b, c], requested: [a, b]) == nil)
        #expect(RemoteTmuxSessionMirror.mirrorTabReorder(current: [a, b, c], requested: [b, c]) == nil)
        // No named tab is present at all → nothing to reorder → nil.
        #expect(RemoteTmuxSessionMirror.mirrorTabReorder(current: [a, b, c], requested: [UUID(), UUID()]) == nil)
        // A full permutation reorders every tab.
        #expect(RemoteTmuxSessionMirror.mirrorTabReorder(current: [a, b, c], requested: [c, a, b]) == [c, a, b])
    }
