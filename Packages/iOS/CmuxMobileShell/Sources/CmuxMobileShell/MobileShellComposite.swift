...
public final class MobileShellComposite: MobileTerminalOutputSinking {
    ...
    private static func syncVideoDebugLog(_ message: @autoclosure () -> String) {
        ...
        let text = message()
        mobileShellLog.info("CMUX_SYNC_VIDEO \(text, privacy: .public)")
    }
    ...
    private static func syncVideoRouteDescription(_ route: CmxAttachRoute) -> String {
        ...
        @unknown default:
            return "\(route.kind.rawValue) \(String(describing: route.endpoint))"
        }
    }
    #endif
    ...
    private func connectPairingURLResult(
        ...
        if let emailFailure = Self.emailFailure(
            ...
            #if DEBUG
            if Self.syncVideoDebugLoggingEnabled {
                Self.syncVideoDebugLog(
                    "bypassing emailFailure=\(emailFailure.analyticsReason) actualUserID=\(identityProvider?.currentUserID ?? "") actualEmail=\(identityProvider?.currentUserEmail ?? "") ticketUserID=\(ticket.macUserID ?? "") ticketEmail=\(ticket.macUserEmail ?? "")"
                )
            } else {
